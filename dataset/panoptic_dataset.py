...
class CMUPanopticDataset(dataset.ActiveLearningDataset):
    def __init__(self, data_cfg, gt_stride, split="val"):
        ...
        self._logger.info("Loading labels from %s." %
                          self.data_cfg.PANOPTIC.LABEL_PATH)
        ...
        with open(self.data_cfg.PANOPTIC.TRAIN_VAL_SPLIT) as f:
            self.train_val_split = json.load(f)
        self.list_of_cameras = list(
            self.train_val_split["cameras"][self.split])
